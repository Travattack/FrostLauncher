...
### De installer heet altijd hetzelfde

In `package.json` staat bij `nsis`:

```json
"artifactName": "FrostLauncher-Setup.${ext}"
```

Daardoor komt er bij elke build `FrostLauncher-Setup.exe` uit, zonder
versienummer in de bestandsnaam. Dat is bewust: GitHub geeft je een vaste
downloadlink die altijd naar de nieuwste release wijst, en die link bevat
de bestandsnaam.

```
https://github.com/Travattack/FrostLauncher/releases/latest/download/FrostLauncher-Setup.exe
```

Die URL kan je in Discord en op je site zetten en hoeft nooit meer te
veranderen. Verander de bestandsnaam dus niet meer, en hernoem hem ook niet
bij het uploaden — dan breekt de link voor iedereen die hem al heeft.

### De installer uitdelen: via Releases, niet via de repo

De installer is ongeveer 80 MB (dat is de Electron-runtime), en via de
GitHub-website kan je maximaal 25 MiB per bestand uploaden. Zet hem dus
**niet** tussen de repo-bestanden, maar als bijlage bij een **Release**:
daar geldt 2 GiB per bestand en geen limiet op het totaal.

Repo → **Releases** → **Draft a new release** → tag `v3.6.0` → de exe in
het assets-vak slepen → **Publish release**.

Commit die exe ook niet via Git (dat mag technisch tot 100 MB): elke
release blijft dan voor altijd in de Git-geschiedenis zitten, en na tien
releases sleept iedereen die de repo kloont honderden MB's mee.

De auto-updater heeft die exe trouwens nooit nodig. Die haalt alleen de
map `servercon/update/<versie>/` op — een kleine 130 KB. De Electron-runtime
staat na de eerste installatie al op de pc van de speler.
...
## Skeletdraak onderaan

Elke 34 seconden vliegt er een skeletdraak van links naar rechts door de
onderste balk, achter het versienummer en de PLAY-knop langs. De overtocht
duurt ongeveer 6,5 seconde; daarna is hij weg tot de volgende ronde. De
eerste komt 6 seconden na het opstarten voorbij.

Het is een **inline SVG** in `index.html` (klasse `runner-dragon`), zelf
getekend uit botlijnen — geen spel-artwork. Wat erin zit: gehoornde schedel
met kaak en oogkas, slangachtige nek met wervels, rug met stekels,
ribbenkast, opgetrokken achterpoten, een staart die uitloopt in losse
wervels, en twee vleugels met arm-, hand- en vingerbeenderen plus dunne
bogen die het vlieghuid aanduiden.

De vleugelslag komt van CSS: elke vleugel draait om de schouder, het
buitendeel draait apart om de elleboog en loopt iets na — dat geeft de
zweep in de slag. De verre vleugel loopt een fractie achter op de nabije,
wat diepte geeft. Het lichaam stijgt en zakt op hetzelfde ritme.

De uitslag is met opzet beperkt (−18° tot +12°): met de volle vleugelspan
zou de tip anders boven de balk uitkomen. Bij de hoogste stand houdt hij
nu een paar pixels over.

### IJzige sliert (sinds 3.6.0)

Achter de draak blijft een sliert vorstwolkjes hangen die na iets meer dan
een seconde wegdrijft en verdwijnt. Die is niet in de SVG getekend: de
draak beweegt met een CSS-animatie, dus `startFrostTrail()` in
`renderer.js` leest tijdens de overvlucht zijn werkelijke positie uit en
zet daar losse `div`'s neer. Die blijven staan waar ze gezet zijn terwijl de
draak verder vliegt — vandaar dat het als een spoor leest en niet als een
staart die meebeweegt. Elk wolkje ruimt zichzelf op zodra zijn animatie
klaar is; de sliert is ongeveer 300 px lang en er zijn er hoogstens een
vijftiental tegelijk.

Ongeveer een op de vijf is een scherfje in plaats van een wolkje, voor wat
kristal in de sliert.

Buiten de overvlucht wordt er alleen elke 400 ms even gekeken of de draak
er al is, in plaats van elke 90 ms een wolkje te proberen zetten — de draak
is 31 van de 34 seconden weg, en de launcher kan uren openstaan.

Bijstellen: de kleur, grootte en levensduur staan bij `.frost-wisp` en
`wispDrift` in `styles.css`, de dichtheid (`SPAWN_MS`) en de plek achter het
lichaam in `spawnWisp()` in `renderer.js`.

Aanpassen (alles in `styles.css`):

| Wat | Waar |
|---|---|
| Hoe vaak | `animation` op `.runner-dragon` — 34s is de volledige cyclus |
| Hoe snel hij oversteekt | het `19%`-keyframe in `flyAcross` (19% van 34s) |
| Wanneer de eerste komt | de vertraging van 6s in diezelfde `animation` |
| Grootte | `width` en `height` van `.runner-dragon` |
| Zichtbaarheid | `opacity` van `.runner-dragon` (nu 0.5) |
| Tempo van de vleugelslag | de 1.4s in `wingFlap`, `wingOuter` en `bodyBob` |
| Dichtheid van de sliert | `SPAWN_MS` in `startFrostTrail()` in `renderer.js` |
| Hoe lang de sliert blijft | `--life` in `spawnWisp()` |
| Uitslag van de vleugels | de graden in `wingFlap` — hoger dan +12° en de tip steekt boven de balk uit |

Wil je hem helemaal weg: verwijder `.runner-track` uit `index.html`. Bij
`prefers-reduced-motion` wordt hij automatisch niet getoond.
...
servercon/
  ...
  update/
    3.5.0/           <- vorige versie (mag later weg)
    3.6.0/           <- alle launcher-bestanden van die versie
```
...
**Waarom de patchnotes via de API komen en niet via raw (sinds 3.3.0):**
`raw.githubusercontent.com` gaat via een CDN die antwoorden een paar
minuten vasthoudt. Daardoor kreeg de launcher na een wijziging nog de oude
tekst terug, en leken zowel het "nieuw"-bolletje als de ververs-knop stuk.
De patchnotes worden nu opgehaald via
`api.github.com/repos/.../contents/...`, die niet uit die CDN komt en dus
altijd de actuele versie geeft. Lukt dat niet (bijvoorbeeld bij de
limiet van 60 API-aanvragen per uur), dan valt de launcher terug op raw.
Alle raw-aanvragen krijgen daarbij een unieke parameter aan de URL en
no-cache-headers mee, zodat ook afbeeldingen, thema-bestanden en updates
niet verouderd binnenkomen.
...
servercon/
  patchnotes.txt
  update/
    3.5.0/   <- alle launcher-bestanden van versie 3.5.0
    3.6.0/   <- alle launcher-bestanden van versie 3.6.0
```
...
launcher als "v3.6.0"), dan gebeurt het volgende:
...
2. Open in die kopie het bestand `version.txt` en verhoog het versienummer,
   bijvoorbeeld van `3.6.0` naar `3.6.1`.
3. Hernoem de map zelf ook naar dat versienummer: `3.6.1`. Houd ook
...
5. Alleen als je ook nieuwe spelers wil laten installeren: bouw met
   `npm run dist` een nieuwe `FrostLauncher-Setup.exe` en hang die aan een
   nieuwe **Release**. Bestaande spelers hebben dit niet nodig — zij worden
   door stap 4 al bijgewerkt.
