...
- `source-config.json` — welke publieke GitHub-repo de bron is, en welke
  logon-server voor de realm-status getest wordt. Vul dit eenmalig in vóór
  je een installer bouwt; spelers hoeven niets in te stellen.
...
servercon/
  ...
  update/
    3.0.1/           <- vorige versie (mag later weg)
    3.1.0/           <- alle launcher-bestanden van die versie
```
...
bouwt**. De `...Path`-velden passen al bij de mapstructuur van
...
servercon/
  patchnotes.txt
  update/
    3.0.1/   <- alle launcher-bestanden van versie 3.0.1
...
launcher als "v3.1.0"), dan gebeurt het volgende:
...
2. Open in die kopie het bestand `version.txt` en verhoog het versienummer,
   bijvoorbeeld van `3.1.0` naar `3.1.1`.
3. Hernoem de map zelf ook naar dat versienummer: `3.1.1`. Houd ook
