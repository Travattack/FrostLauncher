...
Repo → **Releases** → **Draft a new release** → tag `v3.4.0` → de exe in
...
## Skeletpaard onderaan

Elke 34 seconden galoppeert er een skeletpaard van links naar rechts door
de onderste balk, achter het versienummer en de PLAY-knop langs. Het duurt
ongeveer 6,5 seconde en verdwijnt daarna weer tot de volgende ronde. De
eerste doorloop begint 6 seconden na het opstarten.

Het is een **inline SVG** in `index.html` (klasse `runner-horse`), zelf
getekend uit botlijnen — geen spel-artwork. De galop komt van CSS: elk been
draait om zijn schouder of heup, het onderbeen om de knie, en de vier benen
lopen met verschoven fase (`animation-delay`) zodat het samen als een galop
leest. Het lichaam deint mee.

Aanpassen (alles in `styles.css`):

| Wat | Waar |
|---|---|
| Hoe vaak | `animation` op `.runner-horse` — 34s is de volledige cyclus |
| Hoe snel hij oversteekt | het `19%`-keyframe in `gallopAcross` (19% van 34s) |
| Wanneer de eerste komt | de vertraging van 6s in diezelfde `animation` |
| Grootte | `width` en `height` van `.runner-horse` |
| Zichtbaarheid | `opacity` van `.runner-horse` (nu 0.5) |
| Tempo van de benen | de 0.34s in `legSwing`, `shinFold` en `bodyBob` |
...
servercon/
  ...
  update/
    3.3.1/           <- vorige versie (mag later weg)
    3.4.0/           <- alle launcher-bestanden van die versie
```
...
servercon/
  patchnotes.txt
  update/
    3.3.1/   <- alle launcher-bestanden van versie 3.3.1
    3.4.0/   <- alle launcher-bestanden van versie 3.4.0
```
...
launcher als "v3.4.0"), dan gebeurt het volgende:
...
2. Open in die kopie het bestand `version.txt` en verhoog het versienummer,
   bijvoorbeeld van `3.4.0` naar `3.4.1`.
3. Hernoem de map zelf ook naar dat versienummer: `3.4.1`. Houd ook
