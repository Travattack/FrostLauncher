...
// BELANGRIJK: raw.githubusercontent.com wordt via een CDN geserveerd die
// antwoorden een paar minuten vasthoudt. Zonder tegenmaatregel krijgt de
// launcher na een wijziging dus nog de OUDE inhoud terug, en lijkt zowel
// het "nieuw"-bolletje als de ververs-knop stuk. Daarom hangt elke
// raw-aanvraag een unieke parameter aan de URL (de CDN behandelt dat als
// een ander bestand) en sturen we bovendien no-cache-headers mee.
function rawUrl(cfg, filePath) {
  const base = `https://raw.githubusercontent.com/${cfg.owner}/${cfg.repo}/${cfg.branch}/${filePath}`;
  return `${base}?nocache=${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
}
...
async function fetchJson(url) {
  const res = await fetch(url, {
    cache: 'no-store',
    headers: { 'User-Agent': 'wotlk-launcher', ...NO_CACHE_HEADERS },
  });
  if (!res.ok) throw new Error(`Could not fetch ${url} (HTTP ${res.status}).`);
  return res.json();
}

// Haalt een tekstbestand op via de GitHub Contents API in plaats van via
// raw. Die API komt niet van de raw-CDN, dus wat hier terugkomt is altijd
// de actuele versie. Kost een API-aanvraag (60 per uur per IP zonder
// login), vandaar dat alleen de patchnotes deze route gebruiken.
async function fetchTextViaApi(cfg, filePath) {
  ...
  if (!data || data.encoding !== 'base64' || typeof data.content !== 'string') {
    throw new Error(`Unexpected API response for ${filePath}.`);
  }

  return Buffer.from(data.content, 'base64').toString('utf-8');
}

// De patchnotes: eerst via de API (altijd actueel), en als die niet lukt -
// bijvoorbeeld bij een bereikte API-limiet - terugvallen op raw.
async function fetchPatchnotesText(cfg) {
  try {
    return await fetchTextViaApi(cfg, cfg.infoFilePath);
  } catch (e) {
    return fetchText(rawUrl(cfg, cfg.infoFilePath));
  }
}
...
ipcMain.handle('get-patchnotes', async () => {
  ...
  try {
    const content = await fetchPatchnotesText(cfg);
