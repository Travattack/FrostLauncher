...
Repo → **Releases** → **Draft a new release** → tag `v3.2.1` → de exe in
...
servercon/
  ...
  update/
    3.2.0/           <- vorige versie (mag later weg)
    3.2.1/           <- alle launcher-bestanden van die versie
```
...
launcher als "v3.2.1"), dan gebeurt het volgende:
...
2. Open in die kopie het bestand `version.txt` en verhoog het versienummer,
   bijvoorbeeld van `3.2.1` naar `3.2.2`.
3. Hernoem de map zelf ook naar dat versienummer: `3.2.2`. Houd ook
