...
Repo → **Releases** → **Draft a new release** → tag `v3.3.0` → de exe in
...
servercon/
  ...
  update/
    3.2.1/           <- vorige versie (mag later weg)
    3.3.0/           <- alle launcher-bestanden van die versie
```
...
servercon/
  patchnotes.txt
  update/
    3.2.1/   <- alle launcher-bestanden van versie 3.2.1
    3.3.0/   <- alle launcher-bestanden van versie 3.3.0
```
...
launcher als "v3.3.0"), dan gebeurt het volgende:
...
2. Open in die kopie het bestand `version.txt` en verhoog het versienummer,
   bijvoorbeeld van `3.3.0` naar `3.3.1`.
3. Hernoem de map zelf ook naar dat versienummer: `3.3.1`. Houd ook
