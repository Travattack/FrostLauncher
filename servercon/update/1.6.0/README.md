...
Zet zelf een `theme.jpg` in de `assets`-map om de standaard ijslandschap-
illustratie te vervangen. Ontbreekt dat bestand, dan blijft de ingebouwde
illustratie zichtbaar. Tip: liggende foto, minstens 1200x700px.

## Achtergrondmuziek

Zet je eigen (rechtenvrije) epische track in `assets/music/theme.mp3` — zie
`assets/music/LEES_MIJ.txt` voor bronnen. Speelt automatisch in een loop,
stopt zodra je op **SPEEL** drukt. Dempen kan via het luidspreker-icoontje
naast het tandwiel.
...
4. Upload de inhoud van de map `servercon/` uit deze download naar die
   ...
   (met `patchnotes.txt`, `images/`, `update/`) erin.
...
servercon/
  ...
  images/
    voorbeeld.svg
  update/
    1.6.0/   <- alle launcher-bestanden van die versie
```
...
bouwt**. De onderste drie velden passen al bij de mapstructuur van
`servercon/`.
...
servercon/
  patchnotes.txt
  update/
    1.6.0/   <- alle launcher-bestanden van versie 1.6.0
    1.6.1/   <- (als die later uitkomt)
```
...
launcher als "v1.6.0"), dan gebeurt het volgende:
...
2. Open in die kopie het bestand `version.txt` en verhoog het versienummer,
   bijvoorbeeld van `1.6.0` naar `1.6.1`.
3. Hernoem de map zelf ook naar dat versienummer: `1.6.1`. Houd ook
